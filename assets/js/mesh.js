...
function init() {
  fetch('assets/data/knowledge-mesh.json')
    ...
    .catch((err) => {
      console.error('knowledge mesh failed:', err);
      buildFallback();
    });
}

function setPositionAttribute(geometry, values) {
  ...
  throw new Error(`BufferGeometry attribute API missing (three r${THREE_REVISION})`);
}

function createEdgeLines(edgePositions) {
...
function rebuildGraph() {
  ...
  if (edgePositions.length) {
    lineSegments = createEdgeLines(edgePositions);
